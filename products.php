...
// Initialize search variable and type
$search = isset($_GET['search']) ? $_GET['search'] : '';
$type = isset($_GET['type']) ? $_GET['type'] : 'services'; // Default to 'services'

// Prepare the SQL query based on the search input and type
if (!empty($search)) {
    ...
    if ($type === 'services') {
        $query1 = "SELECT service.*, user.nama_lengkap AS penjual, review_counts.jumlah_review, avg_rating.rata_rating 
                    ...
                    LEFT JOIN (SELECT id_service, COUNT(id) AS jumlah_review FROM review GROUP BY id_service) AS review_counts 
                    ON service.id = review_counts.id_service 
                    LEFT JOIN (SELECT id_service, AVG(rating) AS rata_rating FROM review GROUP BY id_service) AS avg_rating 
                    ON service.id = avg_rating.id_service 
                    WHERE service.judul LIKE '%" . $search . "%' AND service.kategori_id = 1";
    ...
    if ($type === 'services') {
        ...
        $query1 = "SELECT * FROM products"; // Get all products if no search term
    } else {
        ...
        <?php foreach ($all as $service): ?>
            <div class="col mb-4">
                    <div class="product-item"
                        onclick="window.location.href='detail-produk.php?idproduk=<?php echo $service['id'] ?>'">
                        <img alt="<?php echo $service['judul'] ?>" height="200"
                            src="/image/product/<?php echo $service['id'] ?>/<?php echo $service['foto'] ?>" class="rounded w-100" style="object-fit: cover;" />
                        <h4><?php echo $service['judul'] ?></h4>
                        <p class="price"><?php echo $service['hargamin'] ?></p>
                        <div class="user-info">
